...
export default function QuestionExtractorPage() {
  ...
  const reextractPreview = async () => {
    ...
    catch (error) { notifications.show({ color: "red", message: error instanceof Error ? error.message : "Ekstraksi ulang gagal." }); }
    finally { setReviewBusy(false); }
  };
  ...
  return <>
    <PageHeader eyebrow="Tools pembelajaran" title="Ekstraktor soal" description="Upload file, tinjau soal dan gambar hasil ekstraksi, lalu setujui untuk masuk ke bank soal." />
    <QuestionExtractorNav active="questions" />
    <Group justify="flex-end" mb="lg"><Button variant="light" color="dark" leftSection={<IconKey size={17}/>} onClick={() => setProviderSettingsOpen(true)}>Pengaturan API key</Button></Group>
    <SimpleGrid cols={{ base: 1, lg: 2 }} spacing="xl" className="extractor-top-grid">
      <Card className="surface-card" p="xl"><Stack gap="lg"><Box><Title order={2} className="section-title">Mulai ekstraksi baru</Title><Text size="sm" c="dimmed" mt={5}>PNG, JPG, PDF, atau DOCX · maksimal 25 MB per file · maksimal {MAX_FILES} file</Text></Box><Select label="Subjek" data={SUBJECTS} value={subject} onChange={setSubject} size="md" />
        ...
        <Button className="primary-action" size="md" leftSection={<IconCloudUpload size={18} />} disabled={!subject || !files.length || readiness?.ready === false} loading={uploading} onClick={submit}>Upload & mulai ekstraksi</Button>
        <Alert color="blue" variant="light" icon={<IconClock size={18} />}>Proses disimpan di database dan akan dilanjutkan setelah server restart. Menutup halaman ini tidak membatalkan proses.</Alert>
      </Stack></Card>
      <Stack gap="md">{selectedJob ? <><JobStatus job={selectedJob}/>{ACTIVE.has(selectedJob.status) ? <Button variant="subtle" color="red" leftSection={<IconPlayerStop size={17}/>} onClick={() => void cancelJob(selectedJob.id).then(() => getJob(selectedJob.id)).then(setSelectedJob)}>Berhenti setelah langkah ini</Button> : ["failed", "cancelled", "completed_with_errors"].includes(selectedJob.status) && <Button variant="light" color="dark" leftSection={<IconPlayerPlay size={17}/>} onClick={() => void resumeJob(selectedJob.id).then(setSelectedJob)}>Lanjutkan file yang belum selesai</Button>}</> : <Card className="surface-card"><Center mih={300}>{loadingJobs ? <Loader color="yellow" /> : <Stack align="center"><IconBook2 size={32} /><Text c="dimmed">Belum ada proses ekstraksi.</Text></Stack>}</Center></Card>}</Stack>
    </SimpleGrid>
