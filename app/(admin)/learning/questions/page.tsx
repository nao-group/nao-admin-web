...
export default function QuestionExtractorPage() {
  ...
  return <>
    ...
    <Drawer opened={Boolean(preview) || loadingPreview} onClose={() => setPreview(null)} title="Question preview" position="right" size="xl" padding="xl">{loadingPreview && !preview ? <Center mih={300}><Loader color="yellow"/></Center> : preview && <Stack gap="xl"><Group><Badge color="dark">{preview.code}</Badge><Badge color="yellow" variant="light">{preview.difficulty}</Badge><Badge color={preview.alignment ? "teal" : "orange"} variant="light">{preview.alignment ? "Alignment ready" : "Alignment pending"}</Badge></Group>{preview.image_url && <Image src={preview.image_url} alt={`Figure for ${preview.code}`} radius="md" fit="contain" mah={320}/>}<Tabs defaultValue={Object.keys(preview.content_en ?? {}).length ? "en" : "zh"}><Tabs.List><Tabs.Tab value="en">English</Tabs.Tab><Tabs.Tab value="zh">中文</Tabs.Tab></Tabs.List><Tabs.Panel value="en" pt="lg"><QuestionBlock content={preview.content_en} preview={preview} language="en"/></Tabs.Panel><Tabs.Panel value="zh" pt="lg"><QuestionBlock content={preview.content_zh} preview={preview} language="zh"/></Tabs.Panel></Tabs><Divider/><Box><Text className="eyebrow">Correct answer</Text><Text fw={800} size="xl">{preview.answer}</Text></Box><Box><Text className="eyebrow">Explanation</Text><Tabs defaultValue={preview.explanation_en ? "en" : "zh"}><Tabs.List><Tabs.Tab value="en">English</Tabs.Tab><Tabs.Tab value="zh">中文</Tabs.Tab></Tabs.List><Tabs.Panel value="en" pt="md"><AlignedText block text={preview.explanation_en || "No English explanation."} language="en" vocab={preview.alignment?.explanation?.vocab_en ? englishVocabToAlignment(preview.alignment.explanation.vocab_en) : preview.alignment?.vocab}/></Tabs.Panel><Tabs.Panel value="zh" pt="md"><AlignedText block text={preview.explanation_zh || "暂无中文解析。"} language="zh" vocab={preview.alignment?.explanation?.vocab_zh ?? preview.alignment?.vocab}/></Tabs.Panel></Tabs></Box></Stack>}</Drawer>
  </>;
}
