...
export function LoginForm() {
  ...
  async function handleRevoke() {
    ...
    try {
      ...
      if (caught instanceof AuthApiError && [400, 404].includes(caught.status)) {
        ...
        setDeviceError(caught instanceof Error ? caught.message : "Gagal mengeluarkan perangkat. Coba lagi.");
      }
  ...
  const deviceLabel = (device: string) => {
    ...
    const platform = device.includes("Windows") ? "Windows" : device.includes("Macintosh") ? "macOS" : device.includes("Android") ? "Android" : /iPhone|iPad/.test(device) ? "iOS" : "Perangkat tidak dikenal";
    return `${browser} di ${platform}`;
  };

  return <Box className="login-page"><Box className="login-ambient login-ambient-one" /><Box className="login-ambient login-ambient-two" /><Card className="login-card" p={{ base: 28, sm: 40 }}><Stack gap={26}><Box><Image src={naoFullDark} alt="NAO Group" width={170} height={70} className="login-logo" priority unoptimized /></Box><Box><Title order={1} className="editorial-title">Selamat datang kembali.</Title><Text c="dimmed" mt={8} lh={1.6}>Masuk untuk mengelola member, finance, dan komunikasi NAO Group.</Text></Box><form onSubmit={handleSubmit}><Stack gap="md"><TextInput label="Email admin" value={email} onChange={(event) => setEmail(event.currentTarget.value)} size="md" autoComplete="email" required /><PasswordInput label="Password" value={password} onChange={(event) => setPassword(event.currentTarget.value)} size="md" autoComplete="current-password" required />{error && <Text size="sm" c="red.7" role="alert">{error}</Text>}<Button type="submit" size="md" fullWidth className="primary-action" rightSection={<IconCheck size={17} />} loading={submitting}>Masuk ke dashboard</Button></Stack></form><Text size="xs" c="dimmed" ta="center">Akses terbatas · khusus tim internal NAO Group</Text></Stack></Card><Text className="login-caption">Satu tempat untuk memahami dan mengembangkan komunitas NAO.</Text>
    <Modal opened={Boolean(maxDevices)} onClose={() => { if (!revoking) { setMaxDevices(null); setDeviceError(""); } }} closeOnClickOutside={!revoking} closeOnEscape={!revoking} withCloseButton={!revoking} title={<Text fw={700} size="lg">Batas perangkat tercapai</Text>} centered size="md">
      <Stack gap="lg"><Text size="sm" c="dimmed">Akun ini sudah aktif di jumlah perangkat maksimum. Pilih satu perangkat untuk dikeluarkan agar Anda dapat melanjutkan ke NAO Admin.</Text><Radio.Group value={selectedSession} onChange={setSelectedSession}><Stack gap="sm">{maxDevices?.sessions.map((session) => <Radio.Card key={session.session_id} value={session.session_id} className="login-device-card" data-checked={selectedSession === session.session_id || undefined} p="md" radius="md"><Group wrap="nowrap"><ThemeIcon color="gray" variant="light" size={40} radius="xl"><IconDeviceDesktop size={19}/></ThemeIcon><Box flex={1} miw={0}><Text size="sm" fw={700}>{deviceLabel(session.device)}</Text><Text size="xs" c="dimmed" mt={3}>Masuk pada {new Date(session.created_at).toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" })}</Text></Box><Radio.Indicator/></Group></Radio.Card>)}</Stack></Radio.Group>{deviceError && <Text size="sm" c="red.7" role="alert">{deviceError}</Text>}<Button fullWidth size="md" className="primary-action" loading={revoking} disabled={!selectedSession} leftSection={<IconLogout size={17}/>} onClick={() => void handleRevoke()}>Keluarkan perangkat & lanjutkan</Button></Stack>
    </Modal>
  </Box>;
}
