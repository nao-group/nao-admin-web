...
export function MetricCard({ label, value, deltaPct, icon: Icon, tone = "navy" }: { label: string; value: string; deltaPct?: number | null; icon: React.ElementType; tone?: "navy" | "gold" | "green" | "purple" }) {
  const hasDelta = deltaPct !== undefined && deltaPct !== null;
  const isDown = hasDelta && deltaPct < 0;
  return <Card className="metric-card" p="lg"><Group justify="space-between" align="flex-start" wrap="nowrap"><Box className="metric-copy"><Text size="xs" fw={600} c="dimmed" tt="uppercase" lts=".05em">{label}</Text><Text className="metric-value">{value}</Text></Box><Box className={`metric-icon ${tone}`}><Icon size={19} stroke={1.8} aria-hidden="true" /></Box></Group>{hasDelta && <Group gap={6} mt="md">{isDown ? <IconTrendingDown size={14} color="#c0392b" aria-hidden="true" /> : <IconTrendingUp size={14} color="#16836b" aria-hidden="true" />}<Text size="xs" c={isDown ? "red.8" : "teal.8"} fw={600}>{deltaPct > 0 ? "+" : ""}{deltaPct}%</Text><Text size="xs" c="dimmed">vs last month</Text></Group>}</Card>;
}
